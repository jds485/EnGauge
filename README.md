...
2. BES_GF_BR_SL.R - Lines 1 to 2152 provide examples of loading the processed data output from Method1Example_GFBR.R for further analysis, processing site water quality data (instead of Water Quality Portal Data) and site precipitation data (instead of NOAA data) using the repository functions, and creating timeseries from the processed datasets for use in model runs. Lines 2153 - end provide examples of using the USGS WRTDS regression method to estimate daily average nutrient concentrations, and using export coefficient models to estimate non-point source pollution loads from ungauged sub-catchments. Note that the WRTDS modified functions script is currently not in this repository because I'm waiting on a reply from the USGS about how is best to host these functions (see [EGRET GitHub issues](https://github.com/USGS-R/EGRET/issues/251)). A text file containing my modified WRTDS functions is available to download from that issue, and you can paste that into an R script and save it to run this example. Estimated runtime is less than 2 hours. Estimated RAM is about 25 GB.
